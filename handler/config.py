...
def decode_product_list(json_data):
    ...
    return {
        int(e['id']): (e['name'], float(e['price']), URL_BASE + e['image_url'])
        for e in json_data}
...
MSG_UNKOWN_CODE = {'heading': 'Lern2Play',
                   'text': 'Nope, den Code gibts\n nicht. Echt nicht!'}
MSG_DECLINE = {'heading': 'Abbruch',
               'text': 'Dann eben nicht.\nBeginne von vorn!'}
MSG_ACCEPT_NO_USER = {'heading': 'Stop! Fehler!',
                      'text': 'Wer kauft hier was?\nScanne deinen Namen.'}
MSG_ACCEPT_NO_PRODUCTS = {'heading': 'Stop! Fehler!',
                          'text': 'Nichts gekauft?\nGlaub ich nicht...'}
...
MSG_SYNC_ON = {'heading': 'Syncing...',
               'text': 'Mach ma leisure,\ngeht gleich weiter.'}
MSG_SYNC_DELAY = DEFAULT_MESSAGE_DELAY
MSG_EXIT = {'heading': u'Ok tschöö...',
            'text': 'Why? Why are you\ndoing this to me?'}
MSG_BUY_ON = lambda name: {'heading': str(name),
                           'text': "You're my favourite\ncustomer", 'delay': 2}
MSG_BUY_OFF = {'delay': 2}  # need a delay -> dunno why
MSG_BUY_RETRY = {'heading': 'Retrying...',
                 'text': 'Wiederhole Vorgang.\nIntranet down?'}
MSG_BUY_RETRY_WAIT = 10
MSG_BUY_FAILED = {'heading': 'Nope, dude!',
                  'text': 'Du kannst nichts\nkaufen. Zumindest im\nMoment. Hello IT?',
                  'delay': DEFAULT_MESSAGE_DELAY}
MSG_SYNC_SUCCESS = {'heading': 'Sync ok',
                    'text': u'Getränke sind wieder\nup to date'}
MSG_SYNC_FAILED = {'heading': 'Sync failed',
                   'text': 'Irgendwas ist schief\ngelaufen'}
