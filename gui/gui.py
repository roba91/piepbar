...
class Gui(object):
    def __init__(self, screen):
        ...
        self._idle_surface = pygame.transform.scale(self._idle_surface,
        ...
        self._message_bg = pygame.transform.scale(self._message_bg,
                                                  (self._res_x, self._res_y))
        self._message_surf = self._message_bg.copy()

        self._main_bg = pygame.image.load(MAIN_PATH)
        self._main_bg.convert()
        self._main_bg = pygame.transform.scale(self._main_bg,
                                               (self._res_x, self._res_y))
        ...
        self._avatar_surf = self._aspect_scale(self._avatar_surf, (
        self._x_unit * 1.9, self._x_unit * 1.9))

        self._drink_img_surf = pygame.image.load(NODRINK_PATH)
        self._drink_img_surf.convert()
        self._drink_img_surf = self._aspect_scale(self._avatar_surf, (
        self._x_unit * 1.9, self._x_unit * 1.9))
    ...
    def set_drink_image(self, url):
        ...
        self._drink_img_surf.convert()
        self._drink_img_surf = self._aspect_scale(self._drink_img_surf, (
        self._x_unit * 1.9, self._x_unit * 1.9))
    ...
    def update(self, name=None, drinks=None, total=0, bill=(0, 0)):
        ...
        if name:
            ...
            name_surf = self._aspect_scale(name_surf,
                                           (self._x_unit * 8, self._y_unit * 2))
        ...
        if drinks:
            ...
            for i, drink in enumerate(drinks[:-9:-1]):
                if len(drinks) > 8 and i == 7:
                    drink_surf = self._font.render(
                        "und noch %s mehr" % (len(drinks) - 7), True,
                        self._text_bg_color)
                    price_surf = self._font.render("...", True,
                                                   self._text_bg_color)
                else:
                    if i == 0:
                        drink_surf = self._font.render(drink[0], True,
                                                       self._text_fg_color)
                        price_surf = self._font.render("%.2f" % drink[1], True,
                                                       self._text_fg_color)
                ...
                drink_surf = self._aspect_scale(drink_surf, (
                5 * self._x_unit * 0.95, drink_height))
                price_surf = self._aspect_scale(price_surf, (
                2 * self._x_unit * 0.95, drink_height))

                self._main_surface.blit(drink_surf, (
                8 * self._x_unit - drink_surf.get_width() - self._x_unit * 0.1,
                (7 * (self._res_y / 8)) - ((i + 1) * drink_height)))
                self._main_surface.blit(price_surf, (
                self._res_x - price_surf.get_width(),
                (7 * (self._res_y / 8)) - ((i + 1) * drink_height)))

            self._main_surface.blit(self._drink_img_surf, (self._x_unit * 0.2,
                                                           14 * self._y_unit * 0.95 - self._drink_img_surf.get_height()))

        total_surf = self._font.render("Summe:", True, self._text_fg_color)
        total_surf = self._aspect_scale(total_surf,
                                        (3 * self._x_unit, 2 * self._y_unit))
        self._main_surface.blit(total_surf, (
        (8 * self._x_unit - total_surf.get_width()), 14 * self._y_unit))

        total_surf = self._font.render("%.2f" % total, True,
                                       self._text_fg_color)
        total_surf = self._aspect_scale(total_surf,
                                        (2 * self._x_unit, 2 * self._y_unit))
        self._main_surface.blit(total_surf, (
        (self._res_x - total_surf.get_width()), 14 * self._y_unit))

        bill_surf = self._font.render("Laufende Rechnung: %.2f" % bill[0], True,
                                      self._text_fg_color)
        bill_surf = self._aspect_scale(bill_surf,
                                       (4 * self._x_unit, 1 * self._y_unit))
        self._main_surface.blit(bill_surf,
                                (0.1 * self._x_unit, 14 * self._y_unit))
        bill_surf = self._font.render("Offene Rechnungen: %.2f" % bill[1], True,
                                      self._text_fg_color)
        bill_surf = self._aspect_scale(bill_surf,
                                       (4 * self._x_unit, 1 * self._y_unit))
        self._main_surface.blit(bill_surf,
                                (0.1 * self._x_unit, 15 * self._y_unit))

        self._draw()

    def message(self, text, heading=None, delay=DEFAULT_MESSAGE_DELAY,
                align=DEFAULT_MESSAGE_ALIGN):
    ...
    def message_on(self, text, heading=None, align=DEFAULT_MESSAGE_ALIGN,
        ...
        try:
            self._message_surf.blit(render_textrect( \
                heading, self._message_head_font,
                pygame.Rect((0, 0), (6 * self._x_unit, 2 * self._y_unit)), \
                self._text_fg_color, pygame.Color('#00000000'), 1 \
                ), (2 * self._x_unit, 4 * self._y_unit))

            self._message_surf.blit(render_textrect( \
                text, self._message_font,
                pygame.Rect((0, 0), (6 * self._x_unit, 6 * self._y_unit)), \
                self._text_fg_color, pygame.Color('#00000000'), 1 \
